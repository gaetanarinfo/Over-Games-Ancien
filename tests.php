...
$achatt = $cnx->prepare('SELECT * FROM Guide_Achat ORDER BY id DESC LIMIT ' . $limite . ', ' . $pagination);
$achatt->execute();

$i = $cnx->prepare("SELECT * FROM Guide_Achat ORDER BY id DESC");
$i->execute();
$x = $i->rowCount();
...
function Pagination2($page_actuelle2, $nb_pages2, $vers=3, $premier=1){
   
	$pagination2 = '';

	$lien2 = preg_replace('`%([^d])`', '%%$1', $page2);
	if (!preg_match('`(?<!%)%d`', $page2) ) $page2 .= '%d';
...
if ( ($page_actuelle2-$vers2) > $premier2+1 )
$pagination2 .= '…';

$debut2 = ($page_actuelle2-$vers2)>$premier2 ? $page_actuelle2-$vers2 : $premier2+1;
$fin2 = ($page_actuelle2+$vers2)<=($nb_pages2-$premier2) ? 
	$page_actuelle2+$vers2 : $nb_pages2-$vers2;

for ( $i2=$debut2 ; $i2<=$fin2 ; $i2++ ) {
	...
	if ( ($page_actuelle2+$around2) < $nb_pages2-$premier2 )
	$pagination2 .= '…';
...
// Lien précédent
if ( $page_actuelle2 > 1 )
	$pagination2 .= '« Précédent';
else
	$pagination2 .= '« Précédent';

// Lien début
for ( $i2=1 ; $i2<=$premier2 ; $i2++ ) {
	$pagination2 .= ' ';
	$pagination2 .= ($page_actuelle2==$i2) ? ''.$i2.'' : ''.$i2.'';
}

// Lien fin
$debut2 = $nb_pages2-$premier2+1;
if( $debut2 <= $premier2 ) $debut2 = $premier2+1;
for ( $i=$debut2 ; $i2<=$nb_pages2 ; $i2++ ) {
	$pagination2 .= ' ';
	$pagination2 .= ($page_actuelle2==$i2) ? ''.$i2.'' : ''.$i2.'';
}

// Lien suivant
if ( $page_actuelle2 < $nb_pages2 )
	$pagination2 .= ' Suivant »';
else
	$pagination2 .= ' Suivant »';
	
	
	if( isset($_GET['jeux']) && is_numeric($_GET['jeux']))
	$page2 = $_GET['jeux'];
else
	$page2 = 1;
...
$limite2 = ($page2 - 1) * $pagination2;

$achat = $cnx->prepare('SELECT * FROM Test_Jeux ORDER BY id DESC LIMIT ' . $limite2 . ', ' . $pagination2);
$slider_result4 = $cnx->query("SET NAMES 'utf8'");
$achat->execute();

$i2 = $cnx->prepare("SELECT * FROM Test_Jeux ORDER BY id DESC");
$i2->execute();
$x2 = $i2->rowCount();

$nb_pages2 = ceil($x2 / $pagination2);
if ($nb_pages2>1){
	$paginations2 .= '' . pagination2($page2, $nb_pages2) . '';
}
?>
...
<meta charset="utf-8">
	...
	<title><?php echo $resultat_site_internet10['Titre_Page_Head']; ?> | <?php echo $resultat_site_internet10['Titre_Page']; ?></title>
	<meta name="description" content="<?php echo $resultat_site_internet10['Description_Page']; ?>">
	<meta name="author" content="<?php echo $resultat_site_internet10['Créateur_Page']; ?>">
	<link rel="icon" type="image/png" href="<?php echo $resultat_site_internet10['Favicon_Page']; ?>">
...
<!-- BEGIN FEATURED BACKGROUND ELEMENT (only on the front page, hide when paged) -->
                        <div class="index-background-element" style="z-index: -1"></div>
...
<div class="index-main-wrapper">
	...
	<p align="center">Nos tests en temp réel - Sans compromis</p>
  
    <?php if($_GET['critique'] == "oui")
    { ?>   
    
<div id="bulle1" style="margin-bottom: 10px;margin-top: 21px;" align="center">
                                        <div style="color: #ffffff;font-weight: bold;width: 604px;font-size: 12px;text-align: center;border-radius:3px;padding: 6px 9px;line-height: 1.6;box-shadow: rgb(134 114 114 / 36%) 0px 0px 10px;height: 18px;background-color: rgb(95 95 95 / 36%);"><span class="fa fa-check" style="color: rgb(127, 214, 74);margin-right: 2px;" aria-hidden="true"></span> Votre critique a bien été envoyer vous pouvez désormais la retrouver dans votre espace personnel.</div>
    									</div>
	<?php } ?>
...
<div id="bulle2" style="margin-bottom: 10px;margin-top: 21px;" align="center">
                                        <div style="color: #ffffff;font-weight: bold;width: 604px;font-size: 12px;text-align: center;border-radius:3px;padding: 6px 9px;line-height: 1.6;box-shadow: rgb(134 114 114 / 36%) 0px 0px 10px;height: 18px;background-color: rgb(95 95 95 / 36%);"><span class="fa fa-times" style="color: #D12023;margin-right: 2px;" aria-hidden="true"></span> Votre critique n'a pas été envoyer merci de recommencer.</div>
    									</div>
	<?php } ?>
  ...
  <hr>
    ...
    <div class="condition">
     ...
     <?php include('wp-includes/Php/Slider_Test.php'); ?>
	
	<script type="text/javascript">
	
	jQuery(document).ready(function(){

	jQuery("#item-right").click(function(evt) {
	jQuery("#item-1").fadeOut(200);
	jQuery("#item-2").fadeIn(200);
	jQuery("#item-3").hide(200);
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").show();
	return false;
	})  
	
	jQuery("#item-left-2").click(function(evt) {
	jQuery("#item-2").fadeOut(200);
	jQuery("#item-1").fadeIn(200);
	jQuery("#item-3").hide(200);
	jQuery("#item-4-4").hide();
	jQuery("#item-3-3").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-1-1").show();
	return false;
	})  
	
	jQuery("#item-right-2").click(function(evt) {
	jQuery("#item-2").fadeOut(200);
	jQuery("#item-3").fadeIn(200);
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-3-3").show();
	jQuery("#item-4-4").hide();
	return false;
	})  

	jQuery("#item-left-3").click(function(evt) {
	jQuery("#item-3").fadeOut(200);
	jQuery("#item-2").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-4-4").hide();
	...
	jQuery("#item-right-3").click(function(evt) {
	jQuery("#item-3").fadeOut(200);
	jQuery("#item-4").fadeIn(200);
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-3-3").hide();
	jQuery("#item-4-4").show();
	return false;
	})  
	
	jQuery("#item-left-4").click(function(evt) {
	jQuery("#item-4").fadeOut(200);
	jQuery("#item-3").fadeIn(200);
	jQuery("#item-3-3").show();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	...
	jQuery("#item-right-4").click(function(evt) {
	jQuery("#item-4").fadeOut(200);
	jQuery("#item-5").fadeIn(200);
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-3-3").hide();
	jQuery("#item-4-4").hide();
	jQuery("#item-5-5").show();
	return false;
	})  
	
	jQuery("#item-left-5").click(function(evt) {
	jQuery("#item-5").fadeOut(200);
	jQuery("#item-4").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-4-4").show();
	jQuery("#item-5-5").hide();
	return false;
	})  
	
	jQuery("#item-right-5").click(function(evt) {
	jQuery("#item-5").fadeOut(200);
	jQuery("#item-6").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-4-4").hide();
	jQuery("#item-5-5").hide();
	jQuery("#item-6-6").show();
	return false;
	})  
	
	jQuery("#item-left-6").click(function(evt) {
	jQuery("#item-6").fadeOut(200);
	jQuery("#item-5").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-4-4").hide();
	jQuery("#item-5-5").show();
	jQuery("#item-6-6").hide();
	return false;
	})  
	
	jQuery("#item-right-6").click(function(evt) {
	jQuery("#item-6").fadeOut(200);
	jQuery("#item-7").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-4-4").hide();
	jQuery("#item-5-5").hide();
	jQuery("#item-6-6").hide();
	jQuery("#item-7-7").show();
	return false;
	})  
	
	jQuery("#item-right-7").click(function(evt) {
	jQuery("#item-7").fadeOut(200);
	jQuery("#item-8").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-4-4").hide();
	jQuery("#item-5-5").hide();
	jQuery("#item-6-6").hide();
	jQuery("#item-7-7").hide();
	jQuery("#item-8-8").show();
	return false;
	})  

	jQuery("#item-left-7").click(function(evt) {
	jQuery("#item-7").fadeOut(200);
	jQuery("#item-6").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	...
	jQuery("#item-left-8").click(function(evt) {
	jQuery("#item-8").fadeOut(200);
	jQuery("#item-7").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	...
	jQuery("#item-6-6").hide();
	jQuery("#item-7-7").show();
	jQuery("#item-8-8").hide();
	return false;
	}) 
		
	jQuery("#item-right-8").click(function(evt) {
	jQuery("#item-8").fadeOut(200);
	jQuery("#item-9").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
	jQuery("#item-4-4").hide();
	...
	jQuery("#item-7-7").hide();
	jQuery("#item-8-8").hide();
	jQuery("#item-9-9").show();
	return false;
	})  
		
	jQuery("#item-left-9").click(function(evt) {
	jQuery("#item-9").fadeOut(200);
	jQuery("#item-8").fadeIn(200);
	jQuery("#item-3-3").hide();
	jQuery("#item-1-1").hide();
	jQuery("#item-2-2").hide();
...
<div id="" class="index-loop-wrapper" style="margin-top: 23px;">
        <div class="index-loop-wrapper-inner">
		
		<div style="color: white;font-weight: 600;font-size: 20px;letter-spacing: 0.8px;font-family: monospace;"><i class="fab fa-angellist" aria-hidden="true" style="
    color: darkgoldenrod;
"></i>  AJOUT RÉCENT AU GUIDE D'ACHAT DU SITE INTERNET</div>
    
      
      <?php while($slider_result4 = $achatt->fetch()) { ?>         
 
 		<table width="100%" border="0" id="article-<?php echo $slider_result4['id'] ?>" data-support="<?php echo $slider_result4['id'] ?>">
  <tbody>
    <tr>
      <td style="width: 114px;"><a data-content="<span style='color:#1A4DD7;' class='fa fa-plus'></span> <b><?php echo $slider_result4['souhaits'] ?> Personne(s) souhaites ce jeux</b><br> <span style='color:#931F21;' class='fa fa-heart'></span> <b><?php echo $slider_result4['aime'] ?> Personne(s) qui aime ce jeux</b><br> <span style='color:#31B759;' class='fa fa-flash'></span> <b><?php echo $slider_result4['note'] ?> Personne(s) on noter ce jeux</b>" data-html="true" data-container="body" data-toggle="popover" data-placement="right"><img class="img_guide" src="<?php if(empty($slider_result4['ImageLien'])) { ?>image/Achat/<?php echo $slider_result4['Image'] ?><?php }else{ ?><?php echo $slider_result4['ImageLien'] ?><?php } ?>" style="width: 98px; height: 98 px;border-radius: 5px;cursor: pointer;"></a></td>
      <td style="
    padding: 0 0 0px 0px;
"><span style="color: #ababab;font-size: 16px;font-weight: bold;font-family: &quot;OpenSansCondensedBold&quot;,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif;line-height: 22px;"><a title="Acheter <?php echo $slider_result4['Titre']; ?> chez notre partenaire." target="_blank" href="<?php echo $slider_result4['Lien']; ?>"><?php echo $slider_result4['Titre'] ?></a></span><br><span style="line-height: 22px;color: beige;font-size: 14px;font-family: &quot;OpenSansCondensedBold&quot;,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif;">Mise en ligne : <?php echo $slider_result4['Date'] ?></span><br><span style="color: beige;font-size: 14px;line-height: 22px;font-family: &quot;OpenSansCondensedBold&quot;,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif;"><?php echo $slider_result4['Tags1'] ?></span><br><span style="color: beige;font-size: 14px;line-height: 22px;font-family: &quot;OpenSansCondensedBold&quot;,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif;">Développé par : <?php echo $slider_result4['Developpeur'] ?></span><br>
  <div class="user-action" data-identifier="J3050391005">
  	<?php if(empty($slider_result4['Categorie1'])) { }else{ ?>
              <a class="TagsConsole label pull-left" style="margin-left: 0px; margin-top: 3px; font-weight:bold; font-size:12px;"><?php echo $slider_result4['Categorie1']; ?><?php } ?></a>
              <?php if(empty($slider_result4['Categorie2'])) { }else{ ?>
             <a class="TagsConsole label pull-left" style="margin-left: 3px; margin-top: 3px; font-weight:bold; font-size:12px;"><?php echo $slider_result4['Categorie2']; ?><?php } ?></a>
              <?php if(empty($slider_result4['Categorie3'])) { }else{ ?>
              <a class="TagsConsole label pull-left" style="margin-left: 3px; margin-top: 3px; font-weight:bold; font-size:12px;"><?php echo $slider_result4['Categorie3']; ?><?php } ?></a>
              <?php if(empty($slider_result4['Categorie4'])) { }else{ ?>
              <a class="TagsConsole label pull-left" style="margin-left: 3px; margin-top: 3px; font-weight:bold; font-size:12px; "><?php echo $slider_result4['Categorie4']; ?><?php } ?></a>         
               <?php if(empty($slider_result4['Categorie5'])) { }else{ ?>
              <a class="TagsConsole label pull-left" style="margin-left: 3px; margin-top: 3px; font-weight:bold; font-size:12px; "><?php echo $slider_result4['Categorie5']; ?><?php } ?></a>  
              <?php if(empty($slider_result4['Categorie6'])) { }else{ ?>
              <a class="TagsConsole label pull-left" style="margin-left: 3px; margin-top: 3px; font-weight:bold; font-size:12px;"><?php echo $slider_result4['Categorie6']; ?><?php } ?></a>  
              <?php if(empty($slider_result4['Categorie7'])) { }else{ ?>
              <a class="TagsConsole label pull-left" style="margin-left: 3px; margin-top: 3px; font-weight:bold; font-size:12px;"><?php echo $slider_result4['Categorie7']; ?><?php } ?></a>  
              
               <?php if(empty($_SESSION['email'])) { ?>                                                      
					<div style="margin-left: 11px;margin-top: 6px;" data-must-login="<?php echo $slider_result4['id'] ?>"><a href="#widgettitle" class="icon2" title="Ajouter"><span class="fa fa-plus"></span></a></div>
                    <?php }else{ ?>
                    
                    <?php if(empty($resultat_achat2['id'])) { ?>
                    <div  id="test-ajout-<?php echo $slider_result4['id'] ?>" style="display:none;"></div>
					<div style="margin-left: 11px;margin-top: 6px;" data-must-login="<?php echo $slider_result4['id'] ?>" id="data-ajout-<?php echo $slider_result4['id'] ?>"><a href="" id="ajout-article-jeux-<?php echo $slider_result4['id'] ?>" class="icon2" title="Ajouter"><span class="fa fa-plus"></span></a></div>
					<?php }else{ ?><div data-must-login="<?php echo $slider_result4['id'] ?>"><span style="color:#1A4DD7;" class="fa fa-plus" title="Vous avez ajouter ce jeux"></span></div>
					<?php } ?>
                    <?php } ?>
                    
                    
                    <?php if(empty($_SESSION['email'])) { ?>
                    <div><a class="icon" title="J'aime" href="#widgettitle"><span class="fa fa-heart"></span></a></div>
                    <?php }else{ ?>
                    
					<?php if(empty($resultat_achat['id'])) { ?>
   
                    <div id="test-<?php echo $slider_result4['id'] ?>" style="display:none;"></div>
					<div data-must-login="<?php echo $slider_result4['id'] ?>" id="data-<?php echo $slider_result4['id'] ?>"><a href="" id="aime-article-jeux-<?php echo $slider_result4['id'] ?>" class="icon" title="J'aime"><span class="fa fa-heart"></span></a></div>
					<?php }else{ ?><div data-must-login="<?php echo $slider_result4['id'] ?>"><span style="color:#931F21;" class="fa fa-heart" title="Vous aimez ce jeux"></span></div>
					<?php } ?>
                    
                    <?php } ?>
                    
                    <?php if(empty($_SESSION['email'])) { ?>
                    <div><a class="icon" title="Note ce jeux" href="#widgettitle"><span class="fa fa-flash"></span></a></div>
                    <?php }else{ ?>
                    <?php if(empty($resultat_achat3['email'])) { ?>
                    <div data-must-login="<?php echo $slider_result4['id'] ?>"><a href="" class="icon3" title="Note ce jeux" data-toggle="modal" data-target="#note-<?php echo $slider_result4['id'] ?>"><span class="fa fa-flash"></span></a></div>
                   	<?php }else{ ?>
					<div data-must-login="<?php echo $slider_result4['id'] ?>"><a href="" style="color:#31B759;" title="Vous avez noter ce jeux"><span class="fa fa-flash"></span></a></div>
					<?php } ?>
                   
                    <?php } ?>
					<div id="data-<?php echo $slider_result4['id'] ?>"><a href="" id="masque-article-jeux-<?php echo $slider_result4['id'] ?>"  class="icon4" title="Masquer ce jeux"><span class="fa fa-scissors"></span></a></div>
    ...
    <div class="post-nav-inner">
       ...
       <?php
                   ...
                   ?>
                       ...
                       <a class="page-numbers<?php if($_SERVER['REQUEST_URI'] == "/tests?page=1" OR $_SERVER['REQUEST_URI'] == "/tests") { ?> current<?php } ?>" href="tests?page=1">1</a>  
						<?php } ?>
...
{  ?>
           		...
           		<a class="page-numbers<?= ($_GET['page'] == $i) ? " current" : "" ?>" href="tests?page=<?php echo $i ?>"><?php echo $i ?></a>
         ...
         <?php
						if($nb_pages != $_GET['page']) {
                   ?>
                    <a href="tests?page=<?php echo $ligne1+$ligne2 ?>" class="next page-numbers">
                       &gt;
                    </a>
				<?php } ?>
   </div>
         ...
         <div id="content" class="index-loop-wrapper" style="margin-top: 10px; display: none;">
        
    	</div>
        
        
       <div id="ramdomplateforme" class="index-loop-wrapper" style="margin-top: 10px;">
        <div class="index-loop-wrapper-inner">
        
            <!-- BEGIN HIGHLIGHTED POSTS MARKER -->
            <div class="necessary-wrapper-for-index">
                                <div class="highlighted-posts-marker">
                   Nos Tests ! En avant première.                   
                </div>
                            </div>
...
<div class="loop-index-wrapper">
    <a href="jeux?item=<?php echo $slider_result4['item'] ?>">
        <div class="loop-index-inner">
            ...
            <div class="featured-image" style="background-image:url(<?php echo $slider_result4['image'] ?>);">
            ...
            <div class="title-excerpt-wrapper">
                ...
                <div class="post-excerpt">
                    ...
                    <!-- END TIME + DATE -->
                    <!-- BEGIN EXCERPT -->
                    <?php echo substr($slider_result4['Paragraphe'], 0, 90).'...'; ?>
	...
	?>
                 ...
                 <div class="note" style="margin-left: 0;margin-top: 15px;text-shadow: 2px 2px 3px rgba(163, 155, 155, 0.5);">
                        ...
                        <span title="Noté par la rédaction" style="<?php echo $couleurTexte ?> font-weight: 700;font-size: 37px;"><?php echo $slider_result4['note'] ?></span><div style="font-weight: 700;font-size: 18px;display: inline-block;">&nbsp;&nbsp;Noté par la rédaction</div>
                            </div>
...
<?php } ?>                 
    ...
    <div class="post-nav-inner">
       ...
       <?php
						if($nb_pages2 != $_GET['page']) {
                   ?>
				
               
                       <a class="page-numbers<?php if($_SERVER['REQUEST_URI'] == "/tests?jeux=1" OR $_SERVER['REQUEST_URI'] == "/tests") { ?> current<?php } ?>" href="tests?jeux=1">1</a>  
						<?php } ?>


  <?php for ($i2 = 2 ; $i2 <= $nb_pages2 - 0 ; $i2++)
{  ?>
      
           		<a class="page-numbers<?= ($_GET['jeux'] == $i2) ? " current" : "" ?>" href="tests?jeux=<?php echo $i2 ?>"><?php echo $i2 ?></a>
         ...
         <?php } ?>
          ...
          <?php
                    ...
                    ?>
         <?php
						if($nb_pages2 != $_GET['jeux']) {
                   ?>
                    <a href="tests?jeux=<?php echo $ligne12+$ligne22 ?>" class="next page-numbers">
                       &gt;
                    </a>
				<?php } ?>
   </div>
...
<?php } ?>
    ...
    <?php include('wp-includes/Php/Login_Test.php'); ?>
...
jQuery('#content').load('test-plateforme?plateforme=PC');
jQuery('#content').show();

jQuery('#ramdomplateforme').hide();
jQuery(this).addClass('fullwidth');return false});
jQuery('a#PS3').click(function(){
jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=PS3');
jQuery('#content').show();
jQuery(this).addClass('fullwidth');return false});

jQuery('a#X360').click(function(){
jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=X360');
jQuery('#content').show();
...
jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=PS5');
jQuery('#content').show();
...
jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=WiiU');
jQuery('#content').show();
...
jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=PSP');
jQuery('#content').show();

jQuery('#ramdomplateforme').hide();
jQuery(this).addClass('fullwidth');return false});
jQuery('a#PSV').click(function(){
jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=PSV');
jQuery('#content').show();

jQuery('#ramdomplateforme').hide();
jQuery(this).addClass('fullwidth');return false});
jQuery('a#DS').click(function(){
	jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=DS');
	jQuery('#content').show();
	
jQuery('#ramdomplateforme').hide();
	jQuery(this).addClass('fullwidth');return false});
	jQuery('a#3DS').click(function(){
		jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=3DS');
	jQuery('#content').show();
	
jQuery('#ramdomplateforme').hide();
	jQuery(this).addClass('fullwidth');return false});
	jQuery('a#IPHONE').click(function(){
		jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=IP');
	jQuery('#content').show();
	
jQuery('#ramdomplateforme').hide();
	jQuery(this).addClass('fullwidth');return false});
	jQuery('a#PS2').click(function(){
		jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=PS2');
		jQuery('#content').show();
		
jQuery('#ramdomplateforme').hide();
		jQuery(this).addClass('fullwidth');return false});
		jQuery('a#PS4').click(function(){
			jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=PS4');
		jQuery('#content').show();
		
jQuery('#ramdomplateforme').hide();
		jQuery(this).addClass('fullwidth');return false});
		jQuery('a#XONE').click(function(){
			jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=XBOX');
			jQuery('#content').show();
			
jQuery('#ramdomplateforme').hide();
			jQuery(this).addClass('fullwidth');return false});
			jQuery('a#AND').click(function(){
				jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=AND');
			jQuery('#content').show();
...
jQuery('#ramdomplateforme').hide();
			...
			jQuery('a#Switch').click(function(){
				jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=Switch');
			jQuery('#content').show();
			...
			jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=PS1');
			jQuery('#content').show();
			...
			jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=Wii');
			jQuery('#content').show();
			...
			jQuery(this).addClass('fullwidth');return false});
			jQuery('a#XONE').click(function(){
				jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=XONE');
			jQuery('#content').show();
			
			jQuery(this).addClass('fullwidth');return false});
			jQuery('a#XBOXSX').click(function(){
				jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=XBOXSX');
			jQuery('#content').show();
			
			jQuery(this).addClass('fullwidth');return false});

			jQuery('a#Stadia').click(function(){
				jQuery('#content').fadeIn(500).load('test-plateforme?plateforme=Stadia');
			jQuery('#ramdomplateforme').hide();
			jQuery('#content').show();
...
<?php
$achat2 = $cnx->prepare("SELECT * FROM Guide_Achat ORDER BY DDate DESC");
$actuachat2 = $cnx->query("SET NAMES 'utf8'");
$achat2->execute();
?>  
<?php
$achat3 = $cnx->prepare("SELECT * FROM Guide_Achat ORDER BY DDate DESC");
$actuachat3 = $cnx->query("SET NAMES 'utf8'");
$achat3->execute();
?>  
...
<?php while ($donnees4 = $achat2->fetch()) { ?>
 
<script type="text/javascript">
	jQuery(document).ready(function(){
	jQuery("#aime-article-jeux-<?php echo $donnees4['id'] ?>").click(function(evt) {
	jQuery("#test-<?php echo $donnees4['id'] ?>").load("jeux-article.php?id=<?php echo $donnees4['id'] ?>");
	jQuery("#test-<?php echo $donnees4['id'] ?>").show();
	jQuery("#data-<?php echo $donnees4['id'] ?>").hide();
	return false;
	})  
	})
</script> 
...
<script type="text/javascript">
	jQuery(document).ready(function(){
	jQuery("#masque-article-jeux-<?php echo $donnees4['id'] ?>").click(function(evt) {
	jQuery("#article-<?php echo $donnees4['id'] ?>").hide();
	return false;
	})  
	})
</script> 
...
<script type="text/javascript">
  ...
  var createPopover = function (item, title) {
        ...
        $pop.popover({
            ...
            var current_popover = '#' + jQuery(e.target).attr('aria-describedby');
            var $cur_pop = $(current_popover);
          
            jQuerycur_pop.find('.close').click(function(){
                //console.log('close triggered');
                $pop.popover('hide');
            });
          
            jQuerycur_pop.find('.OK').click(function(){
...
<?php
$achat20 = $cnx->prepare("SELECT * FROM Guide_Achat ORDER BY DDate DESC");
$actuachat20 = $cnx->query("SET NAMES 'utf8'");
$achat20->execute();
?> 

<?php while ($donnees4 = $achat3->fetch()) { ?>
 
<script type="text/javascript">
	jQuery(document).ready(function(){
	jQuery("#ajout-article-jeux-<?php echo $donnees4['id'] ?>").click(function(evt) {
	jQuery("#test-ajout-<?php echo $donnees4['id'] ?>").load("ajout-article-jeux.php?id=<?php echo $donnees4['id'] ?>");
	jQuery("#test-ajout-<?php echo $donnees4['id'] ?>").show();
	jQuery("#data-ajout-<?php echo $donnees4['id'] ?>").hide();
	return false;
	})  
	})
</script> 
...
<?php while ($donnees4 = $achat20->fetch()) { ?>

 <?php
	$critique3 = $cnx->prepare("SELECT * FROM liste_ratings WHERE id = :id ORDER BY DDate DESC");
	$critique33 = $cnx->query("SET NAMES 'utf8'");
	$critique3->execute(array(":id" => $donnees4['id']));
	$resultat_visiteur = $critique3->fetch();
	?> 

<div class="modal fade" id="note-<?php echo $donnees4['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
          <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 638px !important;">
              <div class="modal-header">
                ...
                <?php if($donnees4['id'] != $resultat_visiteur['id']) { ?>
                Ecrire une critique sur <?php echo substr($donnees4['Titre'], 0, 40 ), '...' ?>
                <?php }else{ ?>
                Critique sur <?php echo substr($donnees4['Titre'], 0, 40 ), '...' ?>
                <?php } ?>
                </h4>
              </div>
	...
	<div class="span12" align="left">
			<div style="font-weight: bold;font-size: 12px;">Ici je vous propose de noter ce jeu et vous pouvez aussi écrire une critique. Votre ressenti par apport a ce jeu la qualité du jeu mais aussi, donné votre avis pour les futurs joueurs.</div>
				</div>	
			</div>
            <?php } ?>

<div class="row-fluid two-methods login">
	<div class="span6">
    <?php if($donnees4['id'] != $resultat_visiteur['id']) { ?>
    <hr>
    <?php } ?>
    <form action="posted-critique?id=<?php echo $donnees4['id'] ?>" method="post" class="form-light padding-15">
  ...
  <tbody>
    <tr>
      <th scope="col"><div align="center">
        <label for="note">Ta note global de ce jeux :</label></div></th>
      
    </tr>
    <tr>
      <td><div align="center">
        <select name="note-<?php echo $donnees4['id'] ?>" required class="form-control" id="note-<?php echo $donnees4['id'] ?>" style="width: 78px;text-align: center;font-size: 11px;font-weight: bold;padding-left: 19px;height: 26px;padding-top: 4px;">
          <option value=""></option>
          <option value="1/10">1/10</option>
          <option value="1/10">2/10</option>
          <option value="1/10">3/10</option>
          <option value="1/10">4/10</option>
          <option value="1/10">5/10</option>
          <option value="1/10">6/10</option>
          <option value="1/10">7/10</option>
          <option value="1/10">8/10</option>
          <option value="1/10">9/10</option>
          <option value="1/10">10/10</option>
        </select>
      </div></td>
...
<table width="300" border="0" align="center">
  <tbody>
    <tr>
      <th scope="col"><div align="center"><label for="note">La qualité de ce jeux :</label></div></th>
      
    </tr>
    <tr>
      <td><div align="center">
        <select name="qualite-<?php echo $donnees4['id'] ?>" required class="form-control" id="qualite-<?php echo $donnees4['id'] ?>" style="width: 78px;text-align: center;font-size: 11px;font-weight: bold;padding-left: 19px;height: 26px;padding-top: 4px;">
          <option value=""></option>
          <option value="1/10">1/10</option>
          <option value="1/10">2/10</option>
          <option value="1/10">3/10</option>
...
<table width="300" border="0" align="center">
  <tbody>
    <tr>
      <th scope="col"><div align="center"><label for="note">La difficulté de ce jeux :</label></div></th>
      
    </tr>
    <tr>
      <td><div align="center">
        <select name="difficulte-<?php echo $donnees4['id'] ?>" required class="form-control" id="difficulte-<?php echo $donnees4['id'] ?>" style="width: 132px;text-align: center;font-size: 11px;font-weight: bold;height: 27px;padding-top: 3px;">
          <option value=""></option>
...
<table width="300" border="0">
  <tbody>
    <tr>
      <th scope="col"><div align="center">
        <label for="critique">Ta critique de ce jeux :</label></div></th>
      
    </tr>
    <tr>
      <td><div align="center">
        <textarea name="critique-<?php echo $donnees4['id'] ?>" class="form-control" required id="critique-<?php echo $donnees4['id'] ?>" style="margin: 0px 0px 10px; width: 553px; height: 231px;"></textarea>
      </div></td>
...
<br>
<button type="submit" class="btn btn-primary">Envoyer la critique</button>
<?php }else{ ?>
        
		<?php
			$critique2 = $cnx->prepare("SELECT * FROM liste_ratings WHERE id = :id ORDER BY DDate DESC");
			$critique22 = $cnx->query("SET NAMES 'utf8'");
			$critique2->execute(array(":id" => $resultat_visiteur['id']));
        ?> 
        
        <?php while ($donnees3 = $critique2->fetch()) { ?>
																				
		<div id="forms4">

<div id="randomdiv" class="comment" align="left">
		<a>
        			<img style="width: 74px; height: 74px;" class="thumbnail" src="<?php echo $donnees3['prenom'] ?>" alt="">
        		</a>
		<div class="comment-content" style="margin-top: -90px;margin-left: 93px;">
			<a class="username" style="font-size: 10px;line-height: 15px;font-weight: bold;margin-top: 0px;"><?php echo $donnees3['prenom'] ?></a>
			<time datetime="1447067168" class="pull-right" style="font-size: 10px;margin-top: 3px;"><?php echo AffDate($donnees3['DDate']) ?></time>
			<p style="font-size: 10px;line-height: 15px;font-weight: bold;margin-top: 0px;"><?php echo $donnees3['critique'] ?></p>
		</div>
		
<hr style="border-bottom: 1px solid rgba(138, 79, 79, 0.28) !important; margin-top: 49px;">
  
    
    <div align="center" style="margin-top: -9px;">
    <span style="font-size: 11.5px;font-weight: bold;border-bottom: 3px solid #2196F3;margin-top: 0px;padding-bottom: 3px;letter-spacing: 0.5px;font-family: &quot;OpenSansCondensedBold&quot;,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif;">Note global : <?php echo $donnees3['note'] ?></span>
    <div style="
    margin-top: 7px;
"></div><span style="font-size: 11.5px;font-weight: bold;margin-top: 0px;padding-bottom: 3px;letter-spacing: 0.5px;border-bottom: 3px solid #2196F3;font-family: &quot;OpenSansCondensedBold&quot;,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif;">Qualité du jeux : <?php echo $donnees3['qualite'] ?></span>
    <div style="
    margin-top: 7px;
"></div>
<span style="font-size: 11.5px;font-weight: bold;letter-spacing: 0.5px;margin-top: 0px;padding-bottom: 3px;border-bottom: 3px solid #2196F3;font-family: &quot;OpenSansCondensedBold&quot;,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif;">Difficulé du jeux : <?php echo $donnees3['difficulte'] ?></span></div>
    
    <hr style="border-bottom: 1px solid rgba(138, 79, 79, 0.28) !important;"> 
